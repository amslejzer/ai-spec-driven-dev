# Documentation Index

**Version:** 1.1  
**Date:** 2026-03-30  
**Status:** Supporting doc

This index is the main entry point for the methodology library.

## Core Method

- [Core Method](core-method.md): the canonical vendor-neutral process
- [Principles](principles.md): beliefs, constraints, and non-goals behind the method
- [Vendor Strategy](vendor-strategy.md): what belongs in the core versus vendor packs
- [Artifact Lifecycle](artifact-lifecycle.md): how templates, examples, and skills are promoted into this repo
- [PM Integration](pm-integration.md): the lightweight markdown PM framework and how it maps to external tools
- [Glossary](glossary.md): shared terminology used throughout the repository

## Reusable Artifacts

- [Project Bootstrap Template](../templates/project-bootstrap-template.md)
- [Ideation Notes Template](../templates/ideation-notes-template.md)
- [Specification Template](../templates/specification-template.md)
- [Documentation Index Template](../templates/documentation-index-template.md)
- [Roadmap Template](../templates/roadmap-template.md)
- [Task Template](../templates/task-template.md)
- [Implementation Plan Template](../templates/implementation-plan-template.md)
- [Session Summary Template](../templates/session-summary-template.md)

## Worked Material

- [Worked Example](../examples/worked-example/README.md): a compact end-to-end example using the method

## Vendor Packs

- [Vendor Packs Overview](../vendors/README.md)
- [Anthropic Pack](../vendors/anthropic/README.md)

## Reading Order

1. Read the [Core Method](core-method.md)
2. Review the [Principles](principles.md) and [Vendor Strategy](vendor-strategy.md)
3. Skim the templates you expect to use most often
4. Review the worked example
5. Layer in a vendor pack only if it fits your tooling
