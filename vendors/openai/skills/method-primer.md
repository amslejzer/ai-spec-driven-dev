...
Each functional role corresponds to a role prompt in this vendor pack:
...
| 4 (build, code) | code author | code-author |
| 4 (build, non-code) | repo operator | repo-operator |
| any | session closer | session-closer |

Note: the OpenAI pack does not include a dedicated project-setup role. Setup is typically performed manually or in a separate editor session.
...
- **Skill** — an instruction artifact for a single AI role in a specific tool context. In the OpenAI pack, skills are role prompts pasted into a target surface (ChatGPT, Custom GPT, Codex, API).
